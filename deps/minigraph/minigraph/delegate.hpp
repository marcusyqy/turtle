  ...
  void connect_impl(R& v) noexcept {
    ...
    if constexpr (is_not_function_ptr) {
      ...
      function  = [](const void* r, As... as) -> decltype(auto) {
        ...
        constexpr bool is_invokable_const = std::is_invocable_r_v<T, decltype(F)..., std::add_const_t<R>, As...>;
        ...
        using converted_ptr_type = std::
            conditional_t<!is_const && (same_t_nonconst || (!same_t_const && convertible_t_nonconst)), R*, const R*>;
